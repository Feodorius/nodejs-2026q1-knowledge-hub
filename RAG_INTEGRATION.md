...
All RAG endpoints require authentication. Log in with the seeded admin account:
...
Copy the `accessToken` from the response — use it as `<admin-jwt-token>` below.

Any authenticated user (e.g. `viewer` / `Admin123!`) can use search and chat endpoints.
...
Response example:
```json
{ "indexedArticles": 3, "indexedChunks": 9, "vectorCollection": "knowledge_hub_articles" }
```

**6. Sample RAG requests**
...
curl -X POST http://localhost:4000/ai/rag/search \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <admin-jwt-token>" \
  -d '{ "query": "how to build a NestJS application", "limit": 5 }'
```

Semantic search with metadata filter (Technology category only):
```bash
curl -X POST http://localhost:4000/ai/rag/search \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <admin-jwt-token>" \
  -d '{ "query": "database indexing and performance", "limit": 5, "articleStatus": "published", "tags": ["Database"] }'
```
...
curl -X POST http://localhost:4000/ai/rag/chat \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <admin-jwt-token>" \
  -d '{ "question": "What is NestJS and what are its key features?" }'
```
...
curl -X POST http://localhost:4000/ai/rag/chat \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <admin-jwt-token>" \
  -d '{ "question": "How do I connect it to a database?", "conversationId": "<id-from-previous-response>" }'
```
...
Remove an article from the index (use a real articleId from the index response or DB):
```bash
curl -X DELETE http://localhost:4000/ai/rag/index/articles/<articleId> \
  -H "Authorization: Bearer <admin-jwt-token>"
```

Incremental reindex (re-run after any article update — unchanged articles are skipped):
```bash
curl -X POST http://localhost:4000/ai/rag/index \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <admin-jwt-token>" \
...
- **`text-embedding-004` in the assignment spec**: The assignment specifies `text-embedding-004`, but this model is not available on all free-tier API keys. `gemini-embedding-001` (3072 dim) is used as a working alternative — switch via `GEMINI_EMBEDDING_MODEL` in `.env` if needed.
...
- **Qdrant collection dimension mismatch**: If you switch embedding models, delete the existing collection first via the Qdrant dashboard (`http://localhost:6333/dashboard`) before restarting the app, so it is recreated with the correct vector dimensions.
